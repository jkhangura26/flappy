...
# AI settings
state_dim = 4  # Bird's normalized state (y, velocity, nearest pipe info)
...
def get_state():
    ...
    if nearest_pipe:
        ...
        gap_center = (top_pipe.height + bottom_pipe.top) / 2

        return np.array([
            bird_y / SCREEN_HEIGHT,                     # Bird's vertical position
            bird_velocity / 10.0,                      # Bird's vertical velocity
            (top_pipe.x - bird_x) / SCREEN_WIDTH,      # Horizontal distance to the next pipe
            (gap_center - bird_y) / SCREEN_HEIGHT      # Vertical distance to the gap center
        ])
    else:
        return np.array([bird_y / SCREEN_HEIGHT, bird_velocity / 10.0, 1.0, 0.0])
...
while running:
    ...
    reward = 5.0 if not done else -100.0
    
    reward += 0.1  # Small positive reward for surviving a frame
    reward = -10 if bird_rect.top <= 0 or bird_rect.bottom >= SCREEN_HEIGHT else reward
    ...
    epsilon = max(epsilon - 1e-4, epsilon_min)
    # epsilon = max(1.0 - (episode / 500), epsilon_min) # more adaptive
